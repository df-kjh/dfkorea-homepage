...
- 실제 가격 표시는 공급자 응답에서 검증된 총액/통화/낙찰 방식/산식 metadata를 확보하고, 낙찰정보서비스 공개 corpus가 동일 기준의 최종 낙찰로 보수적 필터를 통과할 때만 가능하다. 현재 live enrichment는 이 pricing metadata를 제공하지 않아 실제 공고 가격은 `FORMULA_REVIEW_REQUIRED` 또는 `INCOMPARABLE_CONTRACT`로 남을 수 있다. 공개 corpus도 입찰 분류 식별자·단가/총액·지역 필드가 제한되어 많은 결과가 제외되며, 통계는 15개 미만이면 제공하지 않는다.
