...
- 사양과 필수 인증은 동일 제품 snapshot으로 평가한다. 품목별 후보 행렬에서 UNKNOWN·필수 인증 실패가 없는 최선의 제품을 각각 고른 전체 사양 점수를 먼저 확인한다. 개별 품목이 50%여도 다른 품목을 합친 원점수가 80% 이상이면 그 추천 가능한 조합을 보존한다. 나머지는 최대 네 개 전체 공고 선택안의 최종 적합성(추천 → 검토 → 어려움)을 비교하고 같은 등급에서는 완전히 확인된 근거를 우선한다. 후보의 모든 조합을 열거하지 않는다. 인증 없는 100% 제품이나 비교 가능한 사양만 100%인 UNKNOWN 후보를 추가해도 인증·사양이 확인된 80% 추천 후보를 밀어내지 않는다. 명시적 미보유와 인증 정보 누락(UNKNOWN)을 구분한다. 참고 인증도 다른 품목의 미보유가 누락된 인증 근거를 숨기지 않으며, 품목 배정이 안 된 상태는 사양이 비어 있어도 확인 필요다. 제품 식별자는 표시하지 않는다.
...
- G2B 공식 공고명에 총액·원화/KRW·A값 미적용이 명시되고 국내 물품·단일 품목·적격심사·복수예가 방식이 함께 검증된 좁은 경우 실제 enrichment가 가격 context와 근거를 만든다. 저장된 최근 2년 낙찰 이력과 연결하여 공식 범위/15건 이상 통계를 계산한다. 총액·원화·A값 미적용은 공고명 전체의 제한된 문맥에서 명시적인 긍정 문구로 확인한다. 줄바꿈·쉼표·괄호가 불확실성 표현을 분리하지 않으며 `미적용 대상`은 허용하지만 `미확정`, `미적용 여부`, `미적용 가능`, 조건부·부정·상충 문구는 사실로 인정하지 않는다. 2,048자를 넘는 공고명과 해석되지 않는 뒤 문구는 자르거나 추정하지 않고 가격 검토로 남긴다. 단가·외화·특수산식·A값 적용 여부 불명은 계속 보류한다.
...
- `참여 추천`, `검토 필요`, `참여 어려움`, 분석 대기/진행/부분/실패는 아이콘과 한국어 문구로 구분한다. 분석 행이 없는 공고, 실제 대기 작업, 실행 중인 작업을 각각 `자동 등록 예정`, `분석 대기`, `최신 자료 분석 중`으로 안내하며 작업이 없는 응답을 분석 중으로 표시하거나 자동 조회하지 않는다. 대기/진행/실패 때 이전 점수·추천을 현재 결과로 표시하지 않는다. 회사 프로필은 선택 사항이며, 미등록이어도 사양·문서·가격 분석은 제공하고 참가 조건·인증만 확인 필요로 남는다고 안내한다. 이전 검토의 무효화를 표시하고 검토 완료와 2,000자 내부 메모 저장, 다시 분석, 공식 원문 링크를 제공한다.
...
- `회사 자격 설정 (선택)`에서 회사 정보와 나라장터 등록 여부를 편집한다. 참가 조건 자동 확인에 쓰는 면허·기업구분·직접생산·인증·납품실적은 접근 가능한 토글 아래에 기본으로 접어 둔다. 판정에 사용하지 않는 공급물품 분류의 신규 입력 UI는 제공하지 않지만, 기존 저장값은 전체 PUT에서 그대로 보존하고 만료 등으로 정리가 필요하면 기존 항목을 수정하거나 삭제할 수 있다. 실제 저장할 때는 코드·명칭 중복, 만료, 필수값, 실적 기간/금액을 검증하고 만료일 없음은 명시적으로 선택한다. 전체 PUT에 version을 넣지 않으며 저장 실패 시 초안을 보존한다. 닫기/재열기/공고 전환 뒤 오래된 GET/PUT/검토 응답은 화면에 반영하지 않는다.
...
- 서버 시작 시 `2026-09-01 00:00 KST` 이후 등록된 나라장터 물품·K-apt 공고 중 분석 행이 없는 오래된 50건을 큐에 넣는다. 매분 같은 제한 배치를 보충하므로 backlog를 점진적으로 소진한다. 지원하지 않는 나라장터 공사·용역과 다른 공급자는 제외한다. 공고 행 `FOR UPDATE SKIP LOCKED`, 분석의 tender 고유키와 충돌 무시 insert를 함께 사용하여 재시작·다중 인스턴스의 반복 실행에서도 한 공고당 분석 행을 하나만 유지한다. 새 migration이나 환경변수는 필요하지 않다.
...
- cron은 모두 `Asia/Seoul`, `noOverlap: true`를 적용한다. 기존 매시 정각 공고 수집과 매분 정각 메일/재시도를 유지하고, 매시 02분 10초 fingerprint 갱신 후 최대 5건 분석, 매분 20초 누락 분석 최대 50건 등록 후 최대 2건 분석, 매일 02:15 낙찰 증분 수집, 매시 05~50분 중 5분 간격의 30초에 낙찰 백필/증분 1페이지 재개를 실행한다. 종료 시 7개 작업 모두 stop/destroy한다. 낙찰 작업은 일반 수집 advisory lock을 공유하며 정각을 피한다.
...
- 회사 프로필이 없으면 회사 자격을 미보유로 단정하지 않고 참가 조건·인증을 UNKNOWN으로 표시하며 전체 참여 판정은 최대 `검토 필요`로 제한한다. 사양·문서·가격 분석은 프로필 없이도 유지한다. 제품 DB에 없는 IP 등급·조달 분류의 추정 매핑은 하지 않으며 현재 등록된 수치·인증만 사용한다. 개별 제품명·매칭 목록은 제공하지 않는다.
- 한 분석은 최대 10개 첨부를 처리하며 초과 문서는 출처 확인 필요로 남겨 PARTIAL/FAILED 상태에 반영한다. 실패 작업은 수동 재분석 또는 입력 변경으로 다시 실행한다. 제품 변경 감지는 매시이며, 기준일 이후 기존 지원 공고는 서버 시작과 매분 제한 배치로 자동 등록되므로 공고 수에 따라 완료까지 시간이 걸릴 수 있다. 개찰 이력 갱신 자체는 자동 재분석 트리거가 없으므로 최신 가격 검토 시 수동 재분석이 필요하다. 자격 expiresAt 만료는 자동 경계 감지 대상이며, 상대기간 납품실적의 일별 재평가는 별도 후속 작업이다.
