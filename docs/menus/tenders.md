...
- 나라장터 물품의 전기공사업 면허제한 제외는 `getBidPblancListInfoLicenseLimit` 응답에 의존한다. 배포 전 승인된 실운영 키로 현재 OpenAPI의 license-limit 응답·접근/호출 제한을 확인하고, 응답 필드가 fixture와 같고 제한 초과 없이 수집되는지 운영 검증해야 한다.
