...
- 나라장터 면허제한 operation은 공사·물품·용역 기본 operation이 모두 끝난 뒤 실행해 새 요청이 공공데이터포털 초당 호출 한도를 넘기는 병렬 burst를 만들지 않는다. 공급자 결과 코드 `23`은 1.1초 뒤 한 번만 재시도하고, 권한·키·일일 한도 오류는 자동 재시도하지 않는다. 실패 로그에는 API 키와 공급자 메시지를 제외하고 내부 operation 이름과 숫자 결과 코드만 기록한다.
