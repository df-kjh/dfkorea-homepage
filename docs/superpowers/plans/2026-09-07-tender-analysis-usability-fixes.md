...
- [ ] 백엔드 전체 CI와 계약 테스트를 실행한다.
- [ ] 일회용 PostgreSQL이 가능하면 큐 동시성/통합 테스트를 실행하고, 없으면 해당 경계를 명시한다.
- [ ] 프런트엔드 테스트, 타입 검사, 프로덕션 빌드를 실행한다.
- [ ] 전체 변경 범위를 최종 코드 검토하고 모든 Critical/Important 발견을 해결한다.
- [ ] 로컬 결과만 준비하고 원격 push, 배포, 운영 DB backfill은 수행하지 않는다.
