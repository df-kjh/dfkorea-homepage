...
import { TENDER_SOURCE_OPTIONS } from '@/constants/tender-source'
...
<template>
  <section class="tender-filter" aria-label="입찰 공고 필터">
    <div class="tender-filter__fields">
      ...
      <label><span>출처</span><select v-model="form.source"><option :value="undefined">전체</option><option v-for="option in TENDER_SOURCE_OPTIONS" :key="option.value" :value="option.value">{{ option.label }}</option></select></label>
