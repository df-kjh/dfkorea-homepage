...
<template>
  <section class="tender-list" :aria-label="`${selectedDate} 등록 공고`">
    ...
    <template v-else>
      <div class="hidden lg:block overflow-x-auto"><table class="w-full"><thead><tr><th>관련도·기회</th><th>공고명</th><th>발주기관</th><th>출처</th><th>지역</th><th>유형</th><th>등록일</th><th>입찰 마감</th><th>금액</th></tr></thead><tbody><tr v-for="tender in response.data" :key="tender.id"><td><div class="tender-list__badges"><TenderRelevanceBadge :relevance="tender.relevance" /><TenderOpportunityBadge :opportunity-type="tender.opportunityType" /></div></td><td><strong>{{ tender.title }}</strong><p class="reason">{{ tender.relevanceReasons.map((reason) => reason.keyword).join(', ') }}</p><button :data-test="`tender-details-${tender.id}`" type="button" class="tender-list__details" @click="emit('select', tender)" @keydown.enter.prevent="emit('select', tender)" @keyup.space.prevent="emit('select', tender)">상세 보기</button></td><td>{{ tender.orderingOrganization }}</td><td>{{ sourceLabel[tender.source] }}</td><td>{{ tender.region || '-' }}</td><td>{{ procurementLabel[tender.procurementType] }}</td><td>{{ dateLabel(tender.registeredAt) }}</td><td>{{ dateLabel(tender.bidEndedAt) }}</td><td>{{ formatTenderAmount(tender.estimatedAmount) }}</td></tr></tbody></table></div>
      <div class="lg:hidden space-y-3"><button v-for="tender in response.data" :key="tender.id" type="button" class="tender-list__mobile-card" @click="emit('select', tender)"><div class="flex justify-between gap-2 text-left"><div class="tender-list__badges"><TenderRelevanceBadge :relevance="tender.relevance" /><TenderOpportunityBadge :opportunity-type="tender.opportunityType" /></div><span class="text-xs text-gray-500">{{ sourceLabel[tender.source] }}</span></div><strong class="block mt-2 text-left">{{ tender.title }}</strong><span class="block mt-2 text-left text-sm text-gray-600">{{ tender.orderingOrganization }} · {{ tender.region || '지역 미상' }}</span><span class="block mt-1 text-left text-sm text-gray-600">입찰 마감 {{ dateLabel(tender.bidEndedAt) }}</span></button></div>
