...
<template>
  <section class="tender-list" :aria-label="`${selectedDate} 등록 공고`">
    ...
    <template v-else>
      <div class="hidden lg:block overflow-x-auto"><table class="w-full"><thead><tr><th>관련도</th><th>공고명</th><th>발주기관</th><th>출처</th><th>지역</th><th>유형</th><th>등록일</th><th>금액</th></tr></thead><tbody><tr v-for="tender in response.data" :key="tender.id" class="cursor-pointer" tabindex="0" @click="emit('select', tender)" @keyup.enter="emit('select', tender)"><td><TenderRelevanceBadge :relevance="tender.relevance" /></td><td><strong>{{ tender.title }}</strong><p class="reason">{{ tender.relevanceReasons.map((reason) => reason.keyword).join(', ') }}</p></td><td>{{ tender.orderingOrganization }}</td><td>{{ sourceLabel[tender.source] }}</td><td>{{ tender.region || '-' }}</td><td>{{ procurementLabel[tender.procurementType] }}</td><td>{{ dateLabel(tender.registeredAt) }}</td><td>{{ currency(tender.estimatedAmount) }}</td></tr></tbody></table></div>
      <div class="lg:hidden space-y-3"><button v-for="tender in response.data" :key="tender.id" type="button" class="tender-list__mobile-card" @click="emit('select', tender)"><div class="flex justify-between gap-2 text-left"><TenderRelevanceBadge :relevance="tender.relevance" /><span class="text-xs text-gray-500">{{ sourceLabel[tender.source] }}</span></div><strong class="block mt-2 text-left">{{ tender.title }}</strong><span class="block mt-2 text-left text-sm text-gray-600">{{ tender.orderingOrganization }} · {{ tender.region || '지역 미상' }}</span></button></div>
