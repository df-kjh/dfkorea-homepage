...
import type { TenderMailOAuthStatus, TenderSubscription, UpdateTenderSubscription } from '@/types'

interface Props { modelValue: boolean; subscription: TenderSubscription; loading?: boolean; loaded?: boolean; error?: string | null; mailOAuthStatus?: TenderMailOAuthStatus | null; mailOAuthLoading?: boolean; mailOAuthError?: string | null }
const props = withDefaults(defineProps<Props>(), { loading: false, loaded: true, error: null, mailOAuthStatus: null, mailOAuthLoading: false, mailOAuthError: null })
const emit = defineEmits<{ 'update:modelValue': [value: boolean]; retry: []; retryMailOAuth: []; authorizeMailOAuth: []; save: [subscription: UpdateTenderSubscription] }>()
