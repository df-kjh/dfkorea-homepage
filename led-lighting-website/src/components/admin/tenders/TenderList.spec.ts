...
describe('TenderList', () => {
  it('updates Korean suitability labels on desktop and mobile when compact summaries change', async () => {
    const response = { data: [tender], total: 1, page: 1, pageSize: 20, totalPages: 1 }
    const wrapper = mount(TenderList, {
      props: { response, loading: false, error: null, selectedDate: '2026-08-27' },
    })
    ...
    expect(wrapper.findAll('.suitability-badge').map((b) => b.text())).toEqual([
      '✓참여 추천',
      '✓참여 추천',
    ])
  })
  ...
  it('shows a formatted deadline without urgency styling and opens details from an accessible action', async () => {
    const wrapper = mount(TenderList, {
      props: {
        response: { data: [tender], total: 1, page: 1, pageSize: 20, totalPages: 1 },
        loading: false,
        error: null,
