...
| `status` | varchar | `PENDING`, `SENT`, `FAILED`, `RETRY_SCHEDULED`, or `SKIPPED` |
| `nextRetryAt`, `smtpMessageId`, `sentAt`, `failedAt`, `errorMessage` | timestamptz, varchar, timestamptz, timestamptz, text | Nullable retry and SMTP result fields |
| `createdAt`, `updatedAt` | timestamptz | Audit timestamps |

Indexes: `IDX_tender_mail_delivery_status_next_retry_at` and `IDX_tender_mail_delivery_recipient_target_date`.
