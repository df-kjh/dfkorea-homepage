...
- `1738027500000-ChangeProductFieldsToNumber` temporarily removes array defaults, uses PostgreSQL 16-compatible direct array casts, and restores type-correct defaults. It also uses `IF NOT EXISTS` to reconcile optional Product columns that older deployments created through TypeORM synchronization, so a clean migration run reaches the same final Product schema without changing already-migrated databases.
