...
- `ADMIN_USERNAME`은 3–64자의 영문·숫자·점·밑줄·하이픈, `ADMIN_PASSWORD`는 8자 이상이며 소문자·대문자·숫자·기호 중 최소 3개 문자군을 포함해야 한다. CLI는 serializable transaction과 transaction advisory lock 아래 기존 관리자 0명을 다시 확인하고 bcrypt cost 12 hash만 저장한다. 이미 관리자가 있으면 기존 username/password를 수정하지 않고 성공 종료하므로 재배포에도 안전하다. 값/hash는 출력하지 않는다.
...
LH 전자조달 서버는 현재 `TuringSign RSA Secure CA 2` 중간 인증서를 TLS 체인에 포함하지 않는다. 백엔드 Docker 이미지는 검증된 중간 인증서(`SHA-256 A6:F9:C9:67:EB:8A:A9:28:3A:1C:A6:49:B8:7B:76:47:20:E9:F5:C3:AF:A8:1C:15:06:76:F4:CA:36:E9:8C:F6`, 2030-05-26 만료)를 `NODE_EXTRA_CA_CERTS`로 추가한다. TLS 검증은 계속 활성화하며, 인증서 교체 시 LH 서버의 새 체인과 발급자 지문을 확인한 뒤 저장소 인증서를 갱신한다.
...
5. 위 검증이 끝난 뒤에만 관리자 권한으로 최근 2년 백필을 명시적으로 시작한다. 공개 관리자 기능은 백필 시작과 상태 조회만 제공한다. 상태 조회에서 월별 진행 cursor, 저장·제외 건수, `RUNNING`/`SUCCEEDED`/`PARTIAL`/`FAILED` 상태를 모니터링한다. terminal 공급자 실패는 자동 재시도 없이 정지 상태를 유지하며, 승인·설정을 고친 뒤 내부 service 재개를 연결하는 의도적인 maintenance/code/operator 작업이 필요하다. 관리자 백필 시작은 terminal 실패 재개 기능이 아니다.
6. 백필을 세부 단위로 일시 중지하는 관리자 UI/API는 없다. 이상 징후 때문에 이후 scheduled tick까지 막아야 하면 백필 시작을 미루거나 기존 배포 플랫폼의 backend 정지·롤백·scale-to-zero 절차를 사용한다. 이 조치는 일반 API와 공고 수집·메일 등 모든 backend 작업도 함께 멈추므로 해당 영향 범위와 복구 순서를 확인한다.
